...
namespace BootstrapSupport.HtmlHelpers
{
    public static class PaginiationHelperExtensions
    {
            ...
            string additionalPagerCssClass = "")
        {
            ...
            if (totalPages > 1)
            {
                var li_1 = new TagBuilder("li");
                var a_1 = new TagBuilder("a");
                a_1.MergeAttribute("href", pageUrl(1));
                a_1.SetInnerText("<<");
                li_1.InnerHtml = a_1.ToString();
                ul.InnerHtml += li_1;
            }

            int minPage, maxPage;
            minPage = Math.Max(1, currentPage - 5);
            ...
            if (totalPages > 1)
            {
                var li_end = new TagBuilder("li");
                var a_end = new TagBuilder("a");
                a_end.MergeAttribute("href", pageUrl(totalPages));
                a_end.SetInnerText(">>");
                li_end.InnerHtml = a_end.ToString();
                ul.InnerHtml += li_end;
            }
